            ...
            <tbody>
                @foreach ($users as $user)
                    <tr>
                        ...
                        <td>{{ $user->email }}</td>
                        <td>
                            <!-- Formulário de nível de acesso estilizado -->
                            <form action="{{ route('admin.users.updateAccessLevel', $user->id) }}" method="POST"
                                class="access-level-form">
                                @csrf
                                @method('PATCH')
                                <div class="input-field">
                                    <select name="access_level" onchange="this.form.submit()" class="browser-default"
                                        ...
                                        <option value="user" {{ $user->access_level === 'user' ? 'selected' : '' }}>
                                            Usuário</option>
                                        <option value="vendor" {{ $user->access_level === 'vendor' ? 'selected' : '' }}>
                                            Fornecedor</option>
                                        <option value="admin" {{ $user->access_level === 'admin' ? 'selected' : '' }}>
                        ...
                        <td>
                            ...
                            <!-- Modal de confirmação de exclusão do usuário -->
                            <div id="confirmModal{{ $user->id }}" class="modal">
                                <div class="modal-content">
                                    <h4><i class="material-icons">delete</i> Tem certeza?</h4>
                                    <p>Tem certeza de que deseja remover o usuário <strong>{{ $user->firstName }}
